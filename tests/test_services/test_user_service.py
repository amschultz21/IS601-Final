...
async def test_create_user_with_valid_data(db_session, test_email_service):
    user_data = {
    ...
    user = await UserService.create(db_session, user_data, test_email_service)  # <-- FIXED here
    assert user is not None
    assert user.email == user_data["email"]

# Test creating a user with invalid data
async def test_create_user_with_invalid_data(db_session, test_email_service):
    user_data = {
        "nickname": "",
        "email": "invalidemail",
        "password": "short",
    }
    user = await UserService.create(db_session, user_data, test_email_service)
    assert user is None
...
async def test_register_user_with_valid_data(db_session, test_email_service):
    user_data = {
        "nickname": generate_nickname(),
    ...
    user = await UserService.register_user(db_session, user_data, test_email_service)
    assert user is not None
    assert user.email == user_data["email"]

# Test attempting to register a user with invalid data
async def test_register_user_with_invalid_data(db_session, test_email_service):
    user_data = {
